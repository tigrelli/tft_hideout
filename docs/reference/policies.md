...
## 14. 웹 검색 근거 원칙 (CHAT-17, PM 결정 2026-08-12)

- 배경: 5번 "RAG 근거 원칙"은 내부 DB([검색된 문서], 패치버전 근거)를 전제로 한다. CHAT-16(오프토픽 2차 LLM 검증)으로 "시즌 종료는 언제?"처럼 TFT 관련이지만 내부 RAG로는 답할 수 없는 질문도 파이프라인에 들어오게 되면서, 이런 질문 전용으로 Tavily 웹 검색을 근거로 쓰는 5번째 의도(`general_game_info`)를 CHAT-17에서 신설했다
- 규칙: (1) [웹 검색 결과] 섹션에 없는 내용은 추측 금지, 확인 안 되면 "해당 정보는 확인되지 않았습니다"로 답한다(5번과 동일 원칙) (2) 답변에 반드시 출처 URL을 포함한다 (3) 웹 검색 결과는 라이엇 공식 자료가 아닐 수 있어 단정적 표현 대신 완곡한 표현을 쓴다 (4) 답변에 포함된 출처 URL이 실제 검색 결과 URL과 일치하는지 후처리(`verify_web_citation`)로 사후 검증하고, 불일치 시 경고 문구를 덧붙인다(CHAT-06 근거검증과 동일한 결정론적 사후 점검 원칙)
- 5번 정책과의 관계: 5번은 내부 RAG 4개 의도(조합/아이템/증강체/일반전략) 전용, 14번은 웹 검색 전용(general_game_info) — 근거 형식이 근본적으로 달라 별도 시스템 프롬프트(`WEB_SEARCH_SYSTEM_PROMPT`)를 쓰지만 "추측 금지·근거 명시" 원칙 자체는 동일하게 유지
- 담당 TASK: CHAT-17 | 테스트: TEST-00 사전 시나리오 대상 아님(정책·크리티컬 로직이지만 CHAT-17 신설 시점이 TEST-00 이후라 자체 pytest로 검증 — `test_chat17_web_search.py`)
