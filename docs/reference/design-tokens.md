...
OP=`#D9463D`(레드, FE-03에서 실 데이터 확인 후 추가) · S=`#E5B93D`(골드) · A=`#8C6FE0`(퍼플) · B=`#4C9BE0`(블루) · C=`#8C8C8C`(그레이)

- **FE-03에서 발견(2026-08-04)**: op.gg 실제 응답(`comps.tier_rank` = op.gg `opTier`)에 원래 문서에 없던 `"OP"` 값이 존재함을 확인(S보다 상위 등급). 색상은 미확정 항목 재량으로 레드 계열을 임시 채택 — 최종 확정 필요.
...
| 챗봇 위젯 | 우하단 플로팅(360×600, radius 10) | 하단 고정 바 → 탭 시 전체화면 바텀시트 |
...
- **챗봇 위젯**: collapsed 56×56px 원형 / expanded 360×600px(2026-08-08 PM 요청으로 초기 480~505px 범위에서 상향, FE-09 구현 후속 조정), 헤더 배경 `#4059D9`+흰 텍스트, 사용자 말풍선 `#E5E8FF` radius 10px 우측정렬, 봇 답변 좌측정렬 + 근거 텍스트(Caption, `#666666`)
