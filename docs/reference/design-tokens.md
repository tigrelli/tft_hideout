...
## GNB 로고 텍스트 (PM 결정, 2026-08-04)

디자인가이드 6.1·화면설계서 모바일 와이어프레임은 GNB 로고 텍스트를 "TFT Meta"로 명시하지만, 구버전 플레이스홀더로 보고 실제로는 프로젝트명 그대로 **"TFT Hideout"**을 사용하기로 확정(FE-01/FE-02 세션에서 PM 확인). 원본 문서를 다시 열람할 필요 없이 이 결정을 그대로 따른다.
