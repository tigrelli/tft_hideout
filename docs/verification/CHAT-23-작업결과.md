...
## 배포 후 확인 (2026-08-16, c912686 push 직후)

프로덕션(`https://tft-hideout-backend.onrender.com`)에 20초 간격으로 동일 TFT 질문("챔피언 상점에는 몇 명이 나오나요?")을 반복 질의해 배포 반영을 확인:

- 15:00:47(1차): 구모델 스타일 응답(배포 진행 중, 아직 구모델이 서빙)
- 15:01:09(2차): 폴백 오류("일시적인 오류로...") — Render 재시작(콜드 리스타트) 시점으로 추정
- 15:01:31~15:07:49(3~20차, 총 18회): 신모델 스타일 정상 응답을 연속 수신, 오류 0건(3차 이후 답변 텍스트가 모두 동일한 건 CHAT-08 캐시가 동일 질문+patch_version 키를 재사용했기 때문 — 3차에서 신모델이 생성한 답변이 캐시되고 이후는 캐시 히트)

**결론**: `openai/gpt-oss-120b`가 프로덕션에 정상 배포·서빙 확인됨. 챗봇 전체 중단 리스크 해소.
