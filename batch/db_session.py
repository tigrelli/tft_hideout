...
def get_database_url() -> str:
    ...
    # "postgresql://"/"postgres://" 형태인 경우가 많다. 이 프로젝트는 psycopg2가
    # 아니라 psycopg(v3)만 설치하므로(requirements.txt), SQLAlchemy가 기본값인
    # psycopg2로 해석하지 않도록 명시적으로 +psycopg를 붙여준다(이미 드라이버가
    # 지정돼 있으면 그대로 둠) — 2026-08-04 GitHub Actions 배치에서
    # ModuleNotFoundError: psycopg2로 실제로 겪은 문제(backend/db/session.py와 동일 처리).
