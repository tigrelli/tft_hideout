...
SLANG_DICTIONARY: dict[str, str] = {
    ...
    "얼심": "얼어붙은 심장",
    "죽무": "죽음의 저항",
    # CHAT-31(PM 요청, TEST-11 H13·H14에서 발견 2026-08-19): 아이템 전용으로
    # 좁게 쓰이던 은어 사전을 증강체·일반 커뮤니티 은어까지 확장. "오구먼트"는
    # "오그먼트"(증강체)의 흔한 오타/변형 표기라, 정식 표기로 고쳐두면
    # _KEYWORD_PATTERNS(intent_classification.py)의 augment_recommendation
    # 키워드("증강체|오그먼트")에 자동으로 걸려 1차 분류부터 정상 동작한다
    # (H13, "티에프티 오구먼트가 뭐임?"). "어캐"/"롤체"는 특정 용어 전용이
    # 아니라 범용 인터넷 은어(어캐=어떻게, 롤체=TFT의 구어체 줄임말)라 여러
    # 질문에 광범위하게 도움이 된다.
    "오구먼트": "증강체",
    "어캐": "어떻게",
    "롤체": "TFT",
...
# 게임 수를 묻는 질문)가 위 목록 어디에도 안 걸려 2차 LLM으로 넘어갔고,
# 그 2차 LLM도 잡담으로 오판해 범위 밖으로 거절됨(웹검색으로도 답할 수 있는
# 질문인데 아예 도달하지 못함) — 랭크 시스템 관련 용어를 추가했다.
...
_TFT_DOMAIN_PATTERN = re.compile(
    ...
    r"|효과|효능|스킬|설명|경제|이코노미|스노우볼링|그리핑|랭크|배치고사|배치\s*게임|체력"
)
...
_OFF_TOPIC_CONFIRM_SYSTEM_PROMPT = (
    ...
    "전략 은어·랭크(배치고사·티어 등 랭크 시스템) 등) 'on_topic'을, 게임과 전혀 "
    "무관한 잡담이나 다른 게임 질문이면 'off_topic'을 다른 말 없이 정확히 "
    "하나만 출력해라."
)
