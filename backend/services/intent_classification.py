...
# 궁합 질문, "최대 몇 개/명/번"처럼 고정 상한을 묻는 질문 — PM 실사용 제보로
# 2026-08-20 추가, "아이템을 최대 몇 개까지 줄 수 있나요?"가 "아이템" 키워드로
# item_recommendation 확정→RAG에 이 규칙 설명 문서가 없어 "확인되지 않았습니다"
# 오답)이 있으면 카테고리 키워드가 매칭돼도 곧장 확정하지 않고 2차 LLM
...
_MECHANISM_QUESTION_SIGNAL = re.compile(
    ...
    r"|어울리지\s*않는|안\s*어울리는|최대\s*몇\s*(개|명|번)"
)
...
_SYSTEM_PROMPT = (
    ...
    "/메커니즘 질문(예: 아이템은 어떻게 조합하는지, 챔피언에게 아이템을 최대 몇 개까지"
    " 장착할 수 있는지, 성급이 오르면 스킬도 강해지는지,"
    " 랭크 티어는 몇 단계인지, 배치고사는 몇 판을 해야 하는지, 게임 시작 시 기본"
    " 체력은 얼마인지, 매칭은 어떤 기준인지)뿐 아니라, 특정 메타 데이터가"
    " 필요 없는 일반적인 플레이 테크닉·전략 개념 설명(예: 하이롤을 어떻게 준비하는지,"
    " 로우롤 전략이 무엇인지)도 포함한다. 특정 챔피언/아이템/조합을 추천해달라는"
    " 질문이 아니라 게임 시스템이나 보편적 플레이 개념이 무엇인지 묻는 질문이면"
    " 이 카테고리를 우선 선택하라. 단, '이번 패치에서 뭐가 바뀌었는지'"
