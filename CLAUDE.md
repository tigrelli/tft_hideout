...
8. **커밋/푸시**: **git 커밋과 push는 PM이 완료를 확인한 이후에만 실행한다.** PM 확인 전 커밋/푸시 금지. 커밋 메시지는 10장의 Conventional Commits 규칙 + WBS 코드를 따른다. 예: `feat(FE-03): 티어리스트 홈 페이지 구현`. PR을 올리면 SET-14의 CI 게이트가 테스트를 자동 실행하므로, 실패 시 머지 전에 반드시 수정한다.
