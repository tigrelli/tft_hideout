...
- **근거 문서** (읽기 전용, 저장소 루트에 위치):
  ...
  - `/docs/verification/{WBS코드}-작업결과.md` — **모든 완료 TASK**의 PM 확인용 결과 요약(변경 파일, 완료기준·테스트 충족 근거, 설계 중 새로 확정한 사항 등)을 TASK별 파일로 기록(4장 5번 참고). smoke-tests.md는 SET-* 인프라 1회성 검증 전용으로 별도 유지한다
...
6. **작업결과 기록**: PM에게 제시한 결과 요약을 `docs/verification/{WBS코드}-작업결과.md`로 저장한다(다음 세션이 해당 TASK의 설계 판단·구현 근거를 다시 조사하지 않고 이 파일만 보고 참고·수정할 수 있게 하기 위함). 승인 전 초안 상태로 먼저 저장해도 되며, 승인 후 상태만 갱신한다.
7. **진행현황 업데이트**: PM이 확인(승인)한 뒤에만 `진행현황.md`의 해당 WBS 코드 행 상태를 `완료`로, PM확인 컬럼을 갱신하고 6번에서 저장한 파일 링크를 함께 남긴다. PM이 수정을 요청하면 상태를 `진행중`으로 유지하고 반영 후 다시 5번으로 돌아간다.
8. **커밋/푸시**: **git 커밋과 push는 PM이 완료를 확인한 이후에만 실행한다.** PM 확인 전 커밋/푸시 금지. 커밋은 TASK 단위로 분리(구현 커밋 + 완료처리 커밋)하되, 브랜치는 새로 만들지 않고 현재 작업 브랜치에 계속 쌓는다(아래 브랜치 규칙 참고). 커밋 메시지는 10장의 Conventional Commits 규칙 + WBS 코드를 따른다. 예: `feat(FE-03): 티어리스트 홈 페이지 구현`. PR을 올리면 SET-14의 CI 게이트가 테스트를 자동 실행하므로, 실패 시 머지 전에 반드시 수정한다.
9. **다음 TASK로 이동**: 8번까지 끝난 후에만 다음 TASK를 선택한다.

> 요약: **작업선택(사전 시나리오 확인) → 테스트 작성 → 구현 → 자체검증(테스트 통과) → PM확인요청 → 작업결과 기록(docs/verification) → (승인) → 진행현황.md 갱신 → git commit/push(CI 통과) → 다음 TASK.** 이 순서를 반대로 하지 않는다(특히 커밋을 먼저 하고 나중에 확인받는 방식, 테스트 없이 완료 보고하는 방식 모두 금지).

### 커밋/브랜치 규칙
- 브랜치: `main`(배포 대상) / 작업 브랜치 1개(`task/{마지막 WBS코드}` 형식 유지, 예: `task/DATA-01`) — **TASK마다 새 브랜치를 만들지 않는다.** 여러 TASK를 하나의 작업 브랜치에 순서대로 커밋해 쌓고, PM과 합의한 배치 시점(예: 다음 FE 그룹 착수 전)에 그 브랜치 전체를 `main`으로 한 번에 PR 머지한다. 커밋은 여전히 TASK 단위로 분리해 `git log`로 TASK별 추적이 가능하게 한다.
- 배치 머지 시 PR 본문에 포함된 TASK 목록을 나열한다(`docs/verification/{코드}-작업결과.md` 링크 포함).
- 새 작업 브랜치는 직전 배치가 `main`에 머지된 직후, `main`에서 새로 분기해 시작한다(다음 배치도 fast-forward로 단순하게 머지되도록).
- `develop` 브랜치는 더 이상 사용하지 않는다(초기 몇 개 TASK 이후 실제로도 미사용).
- PR 제목에는 대표 TASK 코드 또는 그룹명을 포함한다.
- PM 승인 없이 `main`에 직접 push하지 않는다(단, PM이 이미 승인한 TASK의 `진행현황.md` 상태 갱신만 담은 문서 전용 커밋은 예외 — 4장 참고).
...
/docs       - 기획 문서, WBS, 진행현황, 스파이크 리포트
  ...
  /docs/verification - SET-* 등 인프라 배포·시크릿 1회성 스모크 테스트 결과(smoke-tests.md) + 모든 완료 TASK의 작업결과 요약({WBS코드}-작업결과.md, 4장 6번)
```
