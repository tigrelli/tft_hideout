...
- **근거 문서** (읽기 전용, 저장소 루트에 위치):
  ...
  - `/docs/source/*.docx` — **각 문서의 최신 확정본 원본** (아래 6종, 세부 rationale·특정 절 대조가 필요할 때만 열람):
    ...
    - `TFT_sLLM_개발설계서_v1.8.docx` — 기술 설계 (아키텍처·DB 스키마·API 명세)
    - `TFT_sLLM_IA_v1.2.docx` — 정보구조(사이트맵·URL·화면-데이터 매핑)
  ...
  - `/docs/archive/*` — 구버전 초안(PRD v1.2, 개발설계서 v1.5·v1.6·v1.7, 화면설계서 v1.0/v1.1, IA v1.0, 요구사항정의서 v1.0, 브레인스토밍.md). **이 폴더는 참조하지 않는다** — 이력 보존용이며 최신 근거가 아니다.
